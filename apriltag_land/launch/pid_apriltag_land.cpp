...
#define ERR_I_LIM_DYNAMIC 0.5     // 动态积分限幅
#define Kp_fine           0.15    // 精细比例系数
#define Ki_fine           0.02    // 精细积分系数
#define Kd_fine           0.05    // 精细微分系数 
#define CTRL_LIM_FINE     0.2     // 精细控制输出限幅(单位：米/秒)
	...
	if(count!=0)
	{
		for(int i = 0; i<count; i++)
		{
			...
			{       
			        ...
			        //ROS_INFO("Raw Tag ID: %d", marker.id);  // 添加在if判断之前
				marker_found = true;
